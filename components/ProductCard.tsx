...
export function ProductCard({ p }: { p: Product }) {
  ...
  return (
    <div className="flex flex-col rounded-[14px] bg-white p-3 shadow-card">
      <button onClick={() => nav(`/product/${p.id}`)}
        className="mb-2.5 flex h-16 items-center justify-center rounded-[9px] bg-gradient-to-br from-[#eef3fb] to-[#e9f5de] text-[22px]">
        {p.imageEmoji || '💧'}
      </button>
      <button onClick={() => nav(`/product/${p.id}`)} className="text-left text-[13.5px] font-bold leading-tight text-ink">
        {p.name}
      </button>
      ...
      <div className="mt-auto flex items-center justify-between">
        ...
        <button onClick={quickAdd}
          className="flex h-[30px] w-[30px] items-center justify-center rounded-[9px] bg-[#e8f5db] text-[16px] font-extrabold text-brand-greendark active:scale-95">
          +
        </button>
      </div>
    </div>
