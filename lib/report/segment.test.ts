...
describe('segmentOfOrder', () => {
  ...
  it.each(['kahati', 'group_buy', 'moq'])('files a %s order under group buy', (buyType) => {
  ...
  it('falls back to the item kinds when the order carries no buy type', () => {
    ...
    expect(segmentOfOrder(legacyKahati)).toBe('groupbuy');
  });
...
describe('partitionBySegment', () => {
  it('splits a mixed week without losing or duplicating an order', () => {
    ...
    const { onhand, groupbuy } = partitionBySegment(orders);

    expect(onhand.map((o) => o.orderNo)).toEqual(['BBG-1', 'BBG-4']);
    expect(groupbuy.map((o) => o.orderNo)).toEqual(['BBG-2', 'BBG-3']);
  });
  ...
  it('preserves the incoming order within each half', () => {
    ...
    // route applies.
    const orders = [
      order({ orderNo: 'BBG-9', buyType: 'kahati' }),
      order({ orderNo: 'BBG-8', buyType: 'kahati' }),
      order({ orderNo: 'BBG-7', buyType: 'kahati' }),
    ];
  ...
  it('returns empty halves for an empty week', () => {
    expect(partitionBySegment([])).toEqual({ onhand: [], groupbuy: [] });
  });
});

describe('SEGMENT_LABEL', () => {
  it('names both halves for sheet titles and on-page headings', () => {
    expect(SEGMENT_LABEL.onhand).toBe('On-Hand');
    expect(SEGMENT_LABEL.groupbuy).toBe('Group Buy / Kahati');
  });
});
