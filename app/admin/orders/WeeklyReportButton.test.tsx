...
const half = (invoice: string, buyType: 'solo' | 'group_buy' | 'kahati'): WeeklyReport => ({
  ...
  counts: { paid: 1, pending: 0, cancelled: 0 }, totals: { usd: 100, php: 5000, packingFee: 0 },
  rows: [{
...
const groupbuy = half('BBG-2600', 'group_buy');
const kahati = half('BBG-2700', 'kahati');

const toasts: string[] = [];

vi.mock('@/lib/api-client', () => ({
  apiGet: vi.fn(async () => ({ monday: SELECTED_MONDAY, segments: { onhand, groupbuy, kahati } })),
...
beforeEach(() => {
  ...
  vi.mocked(apiGet).mockResolvedValue({ monday: SELECTED_MONDAY, segments: { onhand, groupbuy, kahati } });
});
...
describe('WeeklyReportButton', () => {
  ...
  it('offers a download per half of the week', () => {
    ...
    expect(screen.getByRole('button', { name: /kahati/i })).toBeInTheDocument();
  });
  ...
  it('says which half was empty instead of downloading a blank workbook', async () => {
    vi.mocked(apiGet).mockResolvedValue({
      monday: SELECTED_MONDAY, segments: { onhand, groupbuy: emptyHalf, kahati },
    });
    const user = userEvent.setup();
    render(<WeeklyReportButton />);
