...
//
// Counters nobody joined are left running: there is nothing to end. Their number
// comes back in the response so the admin can see the board was not wiped
// wholesale. Customer order statuses are neither read nor written here — closing
...
export const POST = handler(async () => {
  ...
  const { rolled, skippedEmpty } = await rollOpenKahatis(db);
  return ok({
