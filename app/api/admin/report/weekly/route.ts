...
export const GET = handler(async (req: Request) => {
  ...
  for (const it of itemRows) {
    ...
    if (it.kind === 'moq_campaign' && included.length > 0) {
      ...
      list.push({
        ...
        productId: it.kahatiProductId,
        code: reportProductCode(it.kahatiName, it.kahatiSpec, it.kahatiCode), kitSize,
        qty: it.qty,
      ...
      const name = it.name ?? it.moqName ?? it.nameSnapshot;
      const spec = it.spec ?? it.moqSpec ?? it.specSnapshot;
      list.push({
        ...base,
        nameSnapshot: name,
        specSnapshot: spec,
        ...
        code: reportProductCode(name, spec, it.code), kitSize: it.kitSize ?? it.kahatiVialCap,
      });
    }
  ...
  // `segments` is what the page renders and the exports download — on-hand,
  // Group Buy, and Kahati are separate operational reports.
