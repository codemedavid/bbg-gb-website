...
const fetchReport = async (): Promise<{
  report: Half;
  segments: { onhand: Half; groupbuy: Half; kahati: Half };
}> => {
...
describe('GET /api/admin/report/weekly returns separate order modes', () => {
  it('files an on-hand order and a hatian order in separate reports', async () => {
    ...
    expect(segments.kahati.rows.map((r) => r.invoice)).toEqual(['BBG-10002']);
  ...
  it('keeps on-hand stock out of the batch-order rollup', async () => {
    ...
    expect(segments.kahati.productTotals.rows.map((r) => r.name)).toEqual(['Retatrutide']);
    expect(segments.kahati.productTotals.totals.qty).toBe(30);
  });
  ...
  it('still divides a hatian line by its supplier kit size within its half', async () => {
    ...
    const [row] = (await fetchReport()).segments.kahati.productTotals.rows;
  ...
  it('splits the money so each half stands on its own', async () => {
    ...
    expect(segments.kahati.orderCount).toBe(1);
    expect(segments.kahati.totals.php).toBe(27000);
  });
  ...
  it('leaves the other half empty rather than absent when a week is one-sided', async () => {
    ...
    expect(segments.kahati.rows).toEqual([]);
  });
  ...
  it('files a legacy order with a defaulted buy type by its line kinds', async () => {
    ...
    expect(segments.kahati.rows.map((r) => r.invoice)).toEqual(['BBG-10009']);
    expect(segments.onhand.rows).toEqual([]);
  });

  it('keeps Group Buy and Kahati orders in different reports', async () => {
    await seedKahatiOrder('BBG-KAHATI');
    await makeOrder('BBG-GROUPBUY', 'group_buy', '5000');

    const { segments } = await fetchReport();

    expect(segments.groupbuy.rows.map((r) => r.invoice)).toEqual(['BBG-GROUPBUY']);
    expect(segments.kahati.rows.map((r) => r.invoice)).toEqual(['BBG-KAHATI']);
  });
});
